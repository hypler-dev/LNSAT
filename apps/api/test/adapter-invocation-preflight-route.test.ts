...
describe("@lnsat/api BP-0116 adapter invocation preflight route", () => {
  ...
  it("inspects adapter invocation preflight evidence through the read-only Fastify route", async () => {
    const response = await gateway.inject({
      method: adapterInvocationPreflightGatewayContract.method,
      url: adapterInvocationPreflightGatewayContract.path,
      payload: {
        request_id: "req_bp0116_route_adapter_invocation_preflight",
        preflight_request: {
          source_refs: [
            {
              source_ref: "ticket:BP-0116",
              summary:
                "Fastify route exposes Gateway adapter invocation preflight evidence",
            },
          ],
          side_effects: [],
        },
    ...
    expect(body.source_refs).toEqual(
      expect.arrayContaining([
        "ticket:BP-0116: Fastify route exposes Gateway adapter invocation preflight evidence",
      ]),
    );
  });
  ...
  it("maps invalid delegated BP-0114 evidence to 400 without raw echo", async () => {
    const response = await gateway.inject({
      method: adapterInvocationPreflightGatewayContract.method,
      url: adapterInvocationPreflightGatewayContract.path,
      payload: {
        request_id: "req_bp0116_invalid_delegated_preflight",
        preflight_request: {
  ...
  it("preserves observation-only preflight without approvals, rollback refs, invocation, dispatch, execution, or side effects", async () => {
    const response = await gateway.inject({
      method: adapterInvocationPreflightGatewayContract.method,
      url: adapterInvocationPreflightGatewayContract.path,
      payload: {
        request_id: "req_bp0116_observation_preflight",
        preflight_request: {
          preflight_identity: {
          ...
          source_refs: [
            {
              source_ref: "ticket:BP-0116",
              summary: "Route wraps observation-only adapter invocation preflight",
            },
          ],
          live_adapter_invocation_allowed: false,
          live_broker_dispatch_allowed: false,
