...
> assigns graphical device, policy, approval, and fleet management to downstream hosts.
...
LNSAT defines two portable authority/runtime contract surfaces; downstream hosts own the
...
3. **Downstream mobile management** - presents devices, models, policies, capacity,
...
## Downstream Mobile Management Requirements

Downstream graphical mobile-management UI should show:
...
Downstream hosts may present proposal and approval workflows, but they do not themselves
...
3. Read-only downstream mobile inventory, policy, and scheduling previews over
