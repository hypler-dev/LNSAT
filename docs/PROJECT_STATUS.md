...
`lnsat-git-reference` executable, exact profile mount-path handoff,
self-executable binding, strict host/container repository-identity remapping,
bounded Git consequence, private-index cleanup, and canonical result framing.
Six hermetic adapter tests use only a host child process and marked temporary
Git fixtures. No real Docker binary/daemon/socket, image pull/build/run,
...
and the supervisor's exact profile mount-path argument. Host-process tests prove
mapped disposable Git execution, executable and target binding, silent pre-output failure,
fail-closed partial output, canonical result output, and private-index cleanup
without Docker access. No
