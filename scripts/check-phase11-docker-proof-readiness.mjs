...
export function validatePhase11DockerProofReadiness({
  ...
  if (
    exactKeys(
      runManifest,
      [
        "schema_id", "fixture_id", "status", "phase11_complete",
        "execution_authorized", "real_docker_proof", "production_supported",
        "contract", "required_plan_binding_ids", "required_case_ids",
        "required_observation_commitment_ids", "preflight_rejection_ids",
        "postspawn_outcome_unknown_ids", "forbidden_public_evidence_fields",
        "authority_declaration_ids", "authority_stop_ids", "next_gate",
      ],
      "run manifest fixture",
      errors,
    )
  ) {
    if (runManifest.schema_id !== "lnsat.phase11_docker_local_runtime_proof_run_manifest_fixture.schema.v1_0") errors.push("run manifest fixture.schema_id: mismatch");
    if (runManifest.fixture_id !== "phase11-docker-local-runtime-proof-run-manifest-v1") errors.push("run manifest fixture.fixture_id: mismatch");
    if (runManifest.status !== "proposed_source_only_no_runtime_evidence") errors.push("run manifest fixture.status: proposed source-only status required");
    for (const field of ["phase11_complete", "execution_authorized", "real_docker_proof", "production_supported"]) if (runManifest[field] !== false) errors.push(`run manifest fixture.${field}: false required`);
    if (exactKeys(runManifest.contract, ["contract_id", "output", "side_effects", "runtime_execution", "proves_human_authority"], "run manifest fixture.contract", errors)) {
      if (runManifest.contract.contract_id !== "lnsat.docker_local_runtime_proof_run_manifest.v1") errors.push("run manifest fixture.contract.contract_id: mismatch");
      if (runManifest.contract.output !== "canonical_private_run_manifest_digest") errors.push("run manifest fixture.contract.output: mismatch");
      if (!same(runManifest.contract.side_effects, [])) errors.push("run manifest fixture.contract.side_effects: [] required");
      if (runManifest.contract.runtime_execution !== false) errors.push("run manifest fixture.contract.runtime_execution: false required");
      if (runManifest.contract.proves_human_authority !== false) errors.push("run manifest fixture.contract.proves_human_authority: false required");
    }
    for (const [field, expected] of [["required_plan_binding_ids", EXPECTED_BINDINGS], ["required_case_ids", EXPECTED_CASE_IDS], ["required_observation_commitment_ids", EXPECTED_OBSERVATION_COMMITMENT_IDS], ["preflight_rejection_ids", EXPECTED_PREFLIGHT_REJECTION_IDS], ["postspawn_outcome_unknown_ids", EXPECTED_POSTSPAWN_OUTCOME_UNKNOWN_IDS], ["forbidden_public_evidence_fields", EXPECTED_FORBIDDEN_PUBLIC_EVIDENCE_FIELDS], ["authority_declaration_ids", EXPECTED_AUTHORITY_DECLARATION_IDS], ["authority_stop_ids", EXPECTED_AUTHORITY_STOP_IDS]]) if (!same(runManifest[field], expected)) errors.push(`run manifest fixture.${field}: ids or order mismatch`);
    if (runManifest.next_gate !== "separately_authorized_real_disposable_docker_image_and_runtime_proof") errors.push("run manifest fixture.next_gate: closed real-Docker gate required");
  }
  if (
    exactKeys(
      executionHarness,
  ...
  for (const marker of [
    ...
    if (!runManifestModuleSource.includes(marker)) errors.push(`runtime proof run manifest module: missing marker ${marker}`);
  }
  for (const forbidden of [
    "std::process", "Command::new", "std::net", "std::fs", "std::env",
    "std::os::unix", "local_unix_socket", "UnixListener", "UnixStream",
    "lnsat_store", "supervise_docker_local_git_execution_v1",
    "execute_phase11_mapped_disposable_git_commit_v1",
  ]) {
    if (runManifestModuleSource.includes(forbidden)) errors.push(`runtime proof run manifest module: forbidden side-effect marker ${forbidden}`);
  }
